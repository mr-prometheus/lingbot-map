...
echo "===================================================="
echo "LingBot-Map Demo PLY Pipeline"
...
clip_frames = int(round(src_fps * CLIP_DURATION_SEC))
step        = max(1, clip_frames // FRAMES_PER_CLIP)
...
# ── Step 2: Run demo.py on each clip ────────────────────────────────────────
echo ""
echo "[2/3] Running demo.py on $clips_extracted clips..."

success=0
failed=0

for clip_dir in $(ls -d "$TEMP_FRAMES_DIR/$video_stem"/clip_* | sort); do
    clip_id=$(basename "$clip_dir")
    clip_out="$OUTPUT_DIR/$video_stem/$clip_id"
    ply_path="$clip_out/point_cloud.ply"

    if [ -z "$FORCE_FLAG" ] && [ -f "$ply_path" ]; then
        echo "  [SKIP] $clip_id — PLY already exists"
        success=$((success + 1))
        continue
    fi

    mkdir -p "$clip_out"
    echo "  [$clip_id] running demo.py ..."

    python demo.py \
        --model_path       "$MODEL_PATH" \
        --image_folder     "$clip_dir" \
        --save_ply         "$ply_path" \
    ...
    if [ $? -eq 0 ]; then
        echo "  [OK] $clip_id → $ply_path"
        success=$((success + 1))
    else
        echo "  [ERROR] $clip_id failed"
        failed=$((failed + 1))
    fi
done
...
rmdir "$TEMP_FRAMES_DIR" 2>/dev/null

echo ""
echo "===================================================="
echo "Video        : $video_stem"
echo "Clips done   : $success"
echo "Clips failed : $failed"
